...
export function getMoviesInSearchPage(values) {
    ...
    return (dispatch) => {
        ...
        const response = axios(`${ROOT_URL}/movietheatres/getMoviesInSearchPage`, {data:values, method: "post", withCredentials: true}, { headers:
...
export function addMovies(values) {
    console.log("from action AddMovies", values);
    return (dispatch) => {
        console.log("kjhg", values)
        const response = axios.post(`${ROOT_URL}/movies/addMovies`, {data:values, withCredentials:true}, { headers:
            { 'Content-Type': 'application/json', "Accept": "application/json"}  })
            .then(response => {
                console.log(response.data);
                if(response.status === 201 || response.status === 304){swal("Movie Successfully added");
